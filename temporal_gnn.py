...
class TemporalGNN(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        self.replay_classifier = nn.Linear(hidden_dim, num_classes)
    ...
    def forward(self, data, lengths=None):
        ...
        if self.temporal_model == "s4":
            x, replay_blend = self.t_model(x, lengths=lengths)  # (batch * num_nodes, seq_len, hidden_dim)
        else:
            x, _ = self.t_model(x)
        ...
        if self.temporal_pool == "mean":
            ...
            replay_blend = self.temporal_pool_layer(x).squeeze(1)

        for i in range(len(self.gnn_layers)):
            # gnn layer
            if self.g_conv == "egcl":
                x,_,_ = self.gnn_layers[i](
                    x, edge_index, coord, edge_attr=edge_weight.reshape(-1, 1)
                )
            else:
                x = self.gnn_layers[i](
        ...
        replay_blend = replay_blend.reshape(batch, num_nodes, -1)

        # graph pool
        if self.graph_pool == "sum":
            x = torch.sum(x, dim=-2)  # (batch, hidden_dim)
            replay_blend = torch.sum(replay_blend, dim=-2)
        elif self.graph_pool == "mean":
            x = torch.mean(x, dim=-2)
            replay_blend = torch.mean(replay_blend, dim=-2)
        elif self.graph_pool == "max":
            x, _ = torch.max(x, dim=-2)
            replay_blend,_ = torch.max(replay_blend, dim=-2)
        else:
            raise NotImplementedError

        # classifier
        x = self.classifier(x)
        replay_logits = self.replay_classifier(replay_blend)

        return x, replay_logits
