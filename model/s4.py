...
# from model.linear_attention import RWKV
...
class ReplayBuffer:
    ...
    def add(self, state):
        """Add a new state to the replay buffer."""
...
class S4Model(nn.Module):
    ...
    def _softmax_blending(self, z, replay_samples, eps=1e-8):
        """
        Computes the softmax-based blending of replay samples with respect to distance from current output z.
        """
        distances = torch.stack([torch.norm(z - sample) + eps for sample in replay_samples])
        
        distances = distances / distances.max()
        
        exp_distances = torch.exp(-distances)  # Exponentiate the negative distances
        softmax_weights = exp_distances / (exp_distances.sum() + eps)  # Normalize to get softmax weights
        
        softmax_weights = softmax_weights.view(-1, 1, 1, 1)  # Add extra dimensions for broadcasting
        
        replay_samples_stacked = torch.stack(replay_samples)  # Ensure replay_samples is a single tensor
        replay_blend = torch.sum(softmax_weights * replay_samples_stacked, dim=0)
    ...
    def forward(self, x, lengths=None):
        ...
        replay_blend = torch.zeros_like(x)
        for layer_idx, (layer, norm, dropout) in enumerate(zip(self.s4_layers, self.norms, self.dropouts)):
            ...
            if len(self.replay_buffer.buffer) > 0 and self.replay_buffer.buffer[0].shape != z.shape:
                self.replay_buffer.clear()

            # Replay mechanism (only for the last layer output)
            if layer_idx == self.n_layers - 1:  # Only for the last layer
                if len(self.replay_buffer.buffer) > 0:
                    # Filter replay samples that match the shape of z
                    replay_samples = [
                        sample for sample in self.replay_buffer.buffer if sample.shape == z.shape
                    ]
                    if replay_samples:
                        # replay_blend = torch.mean(torch.stack(replay_samples), dim=0)
                        replay_blend = self._softmax_blending(z, replay_samples)
                        # z = (z + replay_blend) / 2
                        z = self.alpha * z + (1 - self.alpha) * replay_blend

                # Store output of the last layer in the replay buffer
                self.replay_buffer.add(z.detach())
        ...
        replay_blend = replay_blend.transpose(-1, -2)
        return x, replay_blend